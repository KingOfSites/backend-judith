...
export type SemanticProvider = { model: string; embed(text: string): Promise<number[]>; classify(question: string): Promise<Area | null> };
...
export async function prepareDocument(source: Source, provider: SemanticProvider, cache: EmbeddingStore, progress: () => Promise<void>) {
  ...
  for (const chunk of chunks) {
    ...
    else { await cache.put(embeddingId, provider.model, vector(await provider.embed(semanticText))); created++; }
...
export async function retrieve(question: string, provider: SemanticProvider, load: (area: Area, model: string) => Promise<Candidate[]>) {
  ...
  const query = vector(await provider.embed(question));
