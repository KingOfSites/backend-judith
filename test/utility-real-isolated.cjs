...
const tests=process.env.UTILITY_EXTRA==='1' ? [mainTests.find(x=>x.id==='final-holdout-suppliers'),...extraTests] : [...mainTests,...extraTests];
async function main(){
 ...
 const result=await verifySupport(tests[0].q,[],answer,[chunk]);
 ...
 for(const fixture of JSON.parse(fs.readFileSync('/app/test/fixtures/utility-scope-regression.json','utf8'))){
 ...
 assert.ok(report.scopeControls.every(x=>x.result.supported===x.expected),'SCOPE_CONTROL_FAILED');assert.ok(report.acceptance.accepted>=3,'UTILITY_TOO_LOW');assert.ok(report.acceptance.unsupportedBlocked,'UNSUPPORTED_LEAK');
}
